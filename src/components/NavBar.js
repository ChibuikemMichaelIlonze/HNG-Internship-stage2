...
const NavBar = ({ setMovies, mode, setMode, isLoading, setIsLoading }) => {
  ...
  return (
    <div
      className={`
        fixed w-full h-30 md:h-10 px-2 sm:px-10 md:px-16 lg:px-28 py-4 md:py-8 flex flex-col md:flex-row justify-between items-center md:gap-5 gap-2
    ...
    >
      ...
      <div className="flex flex-1 justify-between gap-12 items-center">
        ...
        {mode ? (
          <BsSunFill
            className={`
              h-7 w-7 cursor-pointer hover:animate-pulse origin-center fill-white duration-700 ease-linear
            `}
            onClick={handleMode}
          />
        ) : (
          <BsMoonFill
            className={`
              h-7 w-7 cursor-pointer hover:animate-pulse origin-center fill-zinc-500 duration-700 ease-linear
            `}
            onClick={handleMode}
          />
        )}
      ...
      <form onSubmit={searchMovie} className="relative">
        <input
          ...
          className={`
            h-10 pl-4 pr-10 block border-2 rounded-lg focus:outline-none
            ${mode
        ...
        >
          <BsSearch
            className={`
              w-5 h-5 hover:w-7 hover:h-7 duration-200
              ${mode ? "fill-white duration-700 ease-linear" : "fill-zinc-700 duration-700 ease-linear"}
            `}
          />
        </button>
      </form>
